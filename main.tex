...
\textcolor{darkblue}{\huge Why do we say it's the same river anyway?}
...
\vspace{1 cm}
\begin{center}
A computer is a set of physical states \\ \underline{\it that we interpret} as computations.
...
\begin{frame}{Programming languages are how we \underline{\it describe} those computations.}
\Large
\vspace{0.75 cm}
...
\vspace{0.75 cm}
\uncover<3->{\Large Only the last actually pushes the abacus beads.}
\end{center}
\end{frame}
...
\vspace{1.5 cm}
\uncover<2->{\textcolor{darkblue}{Von Neumann on Assembly Language:} ``It is a waste of a valuable scientific computing instrument to use it to do clerical work!''}
\end{center}
\end{frame}
...
\vspace{1 cm}
\uncover<2->{\textcolor{darkorange}{\bf But what about speed?} We often choose programming languages for speed.}
...
\vspace{0.25 cm}
\begin{center}
\uncover<2->{Runs \textcolor{darkorange}{\bf 50$\times$ faster} than pure Python, \textcolor{darkorange}{\bf about equal} to C code ({\tt -O3}).}
\end{center}
\end{frame}
...
\vspace{1 cm}
\begin{center}
Languages aren't slow; interpretations are.

\vspace{1 cm}
But an implementation can be hampered by \\ all the flexibility the language promises.
\end{center}
\end{frame}
...
\large
\vspace{0.5 cm}
It can be helpful to create \textcolor{darkblue}{specialized languages} for \textcolor{darkblue}{narrowly defined problems}.
...
{\large \textcolor{darkblue}{External:} SQL has a distinct syntax from Python; must be quoted.}
...
\vspace{0.5 cm}
{\large \textcolor{darkblue}{Internal (embedded):} SparkSQL is an equivalent language, defined within Python.}
...
\textcolor{darkblue}{\Large My answer:} programming languages are human modes of expression, implemented in other programming languages, all the way down.

\vspace{0.35 cm}
What matters is that it's a coherent set of concepts, not whether or not it's implemented with a parser.
\end{columns}
\end{uncoverenv}
\end{frame}

\begin{frame}[fragile]{Scala: a language for internal domain-specific languages}
\large
\vspace{0.5 cm}
...
\begin{frame}[fragile]{Example: tests in the ScalaTest framework}
\vspace{0.5 cm}

\small
\begin{minted}{scala}
def tests(x: Double, str: String) {
...
\vspace{0.25 cm}
\textcolor{darkorange}{\bf We're not thinking about any of those tricks while reading the test code.}
\end{columns}
\end{uncoverenv}
\end{frame}
...
\begin{frame}[fragile]{Typical collider physics problem \only<1>{in C++}\only<2>{in SQL}}
\large
\vspace{0.5 cm}
...
\vspace{0.75 cm}
\begin{uncoverenv}<2->
This {\it can be} described in SQL's relational framework:
