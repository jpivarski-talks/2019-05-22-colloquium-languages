...
\vspace{0.1 cm}
\begin{center}
\uncover<2->{Some languages are better than others.}
\end{center}
...
\vspace{0.75 cm}
\uncover<3->{\Large Only the last level pushes the abacus beads.}
\end{center}
\end{frame}
...
\vspace{1 cm}
\uncover<2->{\textcolor{darkorange}{\bf But what about speed?} We frequently choose programming languages for speed.}
...
\textcolor{darkblue}{Template strings}
...
\vspace{1 cm}
\begin{uncoverenv}<2->
\textcolor{darkblue}{\Large My answer:} programming languages are human modes of expression, which are implemented in other programming languages, all the way down to logic gates.

\vspace{0.25 cm}
It hardly matters if one of those steps is a library of functions, instead of a parser.
\end{uncoverenv}
\end{frame}
