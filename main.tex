...
\vspace{0.7 cm}
\uncover<2->{Each programmer had to divide tasks and combine results manually and account for failures manually.}
...
\vspace{0.7 cm}
\uncover<2->{In fact, FORTRAN (pre-Fortran 90) wasn't even a good fit to {\it data analysis} problems. It lacks jagged tables just as SQL does.}

\vspace{0.7 cm}
\uncover<3->{The gap was filled with a library: ZEBRA provided a graph of structures and dynamic memory management, but these were also features of Ada, C, Pascal, and PL/I.}
...
\vspace{0.5 cm}
\begin{center}
Languages of $\underbrace{\mbox{non-fork repos}}_{\mbox{\small own work}}$ for $\underbrace{\mbox{GitHub users who also fork {\tt cms-sw/cmssw}}}_{\mbox{\small physicists, specifically CMS}}$
\end{center}
...
\vspace{0.25 cm}
\uncover<3->{What we see here are individuals choosing what they think will be easiest or best.}
\end{frame}
...
\vspace{1 cm}
Find out if the physics community thinks they're helpful.
\end{center}
\end{frame}
